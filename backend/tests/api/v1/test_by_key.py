...
This is how anything that refreshes finds its own tile again. It used to be a
...
async def test_second_write_updates_the_same_item(client: AsyncClient) -> None:
    """Writing again replaces the contents rather than adding a second tile."""
...
async def test_position_is_ignored_after_the_first_write(client: AsyncClient) -> None:
    ...
    Otherwise every update would drag the tile back to where the config says.
    """

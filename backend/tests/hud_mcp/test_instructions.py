...
def test_instructions_state_the_configured_grid() -> None:
    ...
    assert f"{settings.GRID_COLS} columns wide and {settings.GRID_ROWS} rows tall" in instructions
...
def test_instructions_cover_what_a_session_cannot_guess() -> None:
    ...
    for fact in (
        ...
        "show_page",  # that the board holds several screenfuls and turns to one
        "fold_group",  # and that a handful of widgets on one of them fold away
    ):

...
def register(server: MCPServer) -> None:
    ...
    async def set_media_queue(item_id: str, tracks: list[str], start: int = 0) -> str:
        ...
        payload = item.payload.model_copy(update={"tracks": queue, "index": index})
        return await _write(item, payload, "queued")

    @server.tool()
    async def control_media(item_id: str, action: str) -> str:
        """Drive a player: play, pause, stop, next or back.
    ...
    async def set_media_mode(
        ...
        if item is None:
            return f"No media widget {item_id}. Call list_items to see what is there."
        asked = {"loop": loop, "muted": muted, "maximised": maximised}
        given = {name: value for name, value in asked.items() if value is not None}
        if not given:
            return "Nothing to set: pass at least one of loop, muted or maximised"
